...
export default function AdminUploadDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.error('Failed to load admin upload dashboard:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUploadAndScan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Redirect to admin real-time progress page
        router.push(`/admin/scan/${data.scanId}`);
      } else {
    ...
    switch (item.status) {
      case 'COMPLETED':
        return item.combinedScore !== null ? (
          ...
          <span className="px-2 py-0.5 text-xs font-semibold rounded bg-slate-100 text-slate-700">Ready</span>
        );
      case 'PROCESSING':
        return <span className="px-2 py-0.5 text-xs font-semibold rounded bg-indigo-50 text-indigo-700 border border-indigo-205 animate-pulse">Scanning</span>;
      case 'PENDING':
  ...
  return (
    <>
      {/* Title */}
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Document Scan Center (Teacher View)</h2>
        <p className="text-sm text-slate-500">Upload written documents to evaluate potential AI generation risk across enabled providers</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="p-6 pt-0">
            <form onSubmit={handleUploadAndScan} className="space-y-6">
              {uploadError && (
                <div className="p-3 bg-red-50/60 border border-red-200 rounded-lg text-xs text-red-700 font-medium">
                  {uploadError}
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                {providers.length === 0 ? (
                  <div className="p-4 rounded-lg bg-amber-50 border border-amber-100 text-amber-850 text-xs flex items-start space-x-2 leading-relaxed">
                    ...
                    <p>
                      <strong>No active detectors available.</strong> You must enable at least one active provider before running a scan. Please configure this in the Provider Keys panel.
                    </p>
                  </div>
                ) : (
                    ...
                    {providers.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {p.isMockMode && (
                            <span className="text-[9px] font-bold bg-slate-105 text-slate-450 rounded px-1.5 py-0.2 border border-slate-200">
                              Simulated
                            </span>
                          )}
                        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4 text-xs text-slate-655 leading-relaxed">
            <div className="p-3.5 bg-indigo-50 text-indigo-900 border border-indigo-150 rounded-lg flex items-start space-x-2.5">
              ...
              <div className="space-y-1">
                <h4 className="font-bold text-xs">Teacher Scan Context</h4>
                <p>
                  Running a scan from here will log the results under your Administrator account. The results will be visible in the "Recent Reviews" list on your dashboard.
                </p>
              </div>
            </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
          <div>
            ...
            <CardDescription>Previous scans uploaded by you as teacher</CardDescription>
          </div>
        </CardHeader>
                    ...
                    <TableCell className="text-right">
                      {item.status === 'COMPLETED' ? (
                        <Link
                          ...
                          className="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-805 rounded-lg text-xs font-semibold border border-slate-300 transition-colors"
                        >
                        ...
                        <span className="text-xs text-red-600 font-semibold italic">Failed</span>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
