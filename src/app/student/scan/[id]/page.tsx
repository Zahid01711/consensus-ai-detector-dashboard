...
export default function ScanProgressPage() {
    ...
    const pollScanStatus = async () => {
      try {
        ...
        if (scanData.scan.status === 'COMPLETED' || scanData.scan.status === 'FAILED') {
          // Keep the final view for a second, then offer action
          return;
        }
  ...
  const getJobStatusIcon = (status: JobProgress['status']) => {
    switch (status) {
      ...
      default:
        return <Clock className="h-5 w-5 text-slate-405 shrink-0" />;
    }
  };
  ...
  const getJobStatusText = (status: JobProgress['status']) => {
    switch (status) {
      case 'COMPLETED':
        return <span className="text-xs font-semibold text-emerald-705">Completed</span>;
      case 'FAILED':
        return <span className="text-xs font-semibold text-red-750">Failed</span>;
      case 'PROCESSING':
                ...
                >
                  ...
                  <div>
                    {job.status === 'COMPLETED' && job.score !== null ? (
                      ...
                      <span className="text-[10px] text-red-650 max-w-[150px] truncate block" title={job.errorMessage || 'Scan failed'}>
